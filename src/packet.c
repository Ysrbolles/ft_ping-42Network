...
int send_packet()
{
	...
	printf("%ld----- %ld\n ",g_params.time_start.tv_sec, g_params.time_start.tv_usec);
...
long double calc(struct timeval start, struct timeval end)
{
	...
	printf("-----------> start %d \n", start.tv_usec);
	long double startII = start.tv_sec * 1000 + start.tv_usec / 1000;
	long double endII = end.tv_sec * 1000 + end.tv_usec / 1000;

	res = (long double)(endII - startII);
	printf("------> res %Lf\n", res);
	return (res);
}
int get_packet()
